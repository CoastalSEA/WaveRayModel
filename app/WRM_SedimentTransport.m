...
%%
        function transportPlots(mobj)
            ...
            listxt = {'Annual Mean Drift','Binned Mean Drift',...
                      ...
                      'Duration Exceedance','Binned Statistics',...
                      'Absolute Cluster Statistics','Pos/Neg Cluster Statistics',...
                      'Wave Rose Plots'};
                ok = 1;
                while ok>0
...
%%
    methods
        function dst = getDrift(obj,mobj,dst,driftmodel)
            ...
            Qs = zeros(nrec,npnts); Qx = Qs; dQdx = Qs; zR2 = Qs; alpi = Qs;
            ...
            for i=1:npnts                
                ...
                [~,~,~,R2] = runup(ubs,wv.Hs,wv.Tp,1,wv);%for gravel beaches
                zR2(:,i) = R2+wv.swl;                    %runup elevation using 
            ...
            for i=1:npnts   
                ...
                adst = dstable(Qs(:,i),dQdx(:,i),Qx(:,i),zR2(:,i),alpi(:,i),...
...
%%
        function dsprop = setDSproperties(~)
            ...
            dsprop.Variables = struct(...
                ...
                'Unit',{'m^3/s','m^3/s/m','m^3/s','m','rad'},...
                'Label',{'Transport rate (m^3/s)','Transport gradient (m^3/s/m)',...
                               'Transport Rate (m^3/s)','Wave Runup (mOD)',...
                               'Transport direction'},...
